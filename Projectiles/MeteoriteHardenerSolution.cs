...
{
	public class MeteoriteHardenerSolution : ModProjectile
	{
		...
		public override void AI()
		{
			...
			int dustType = DustType<Dusts.MeteoriteHardenerSolution>();
			...
			for (int k = i - size; k <= i + size; k++)
			{
				for (int l = j - size; l <= j + size; l++)
				{
					if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size))
					{
						...
						if (type == 37)
						{
							Main.tile[k, l].type = (ushort)mod.TileType("HardenedMeteoriteOre");
